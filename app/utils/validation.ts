...
export const validateText = (
    ...
    if (minLength && value.trim().length < minLength) {
        return { isValid: false, error: `${fieldName} moet minstens ${minLength} karakters bevatten` };
    }
    ...
    if (pattern && !pattern.test(value)) {
        return { isValid: false, error: options.customMessage || `${fieldName} heeft een ongeldig formaat` };
    }
...
export const validateDate = (
    ...
    if (min === 'today' && date < today) {
        return { isValid: false, error: `${fieldName} moet vandaag of in de toekomst zijn` };
    }
    if (min === 'past' && date >= today) {
        return { isValid: false, error: `${fieldName} moet in het verleden liggen` };
    }
    ...
    if (max === 'today' && date > today) {
        return { isValid: false, error: `${fieldName} kan niet in de toekomst liggen` };
    }
    ...
    if (minAge || maxAge) {
        ...
        if (minAge && actualAge < minAge) {
            return { isValid: false, error: `Je moet minstens ${minAge} jaar oud zijn` };
        }

        if (maxAge && actualAge > maxAge) {
            return { isValid: false, error: `Je mag maximaal ${maxAge} jaar oud zijn` };
        }
    }
