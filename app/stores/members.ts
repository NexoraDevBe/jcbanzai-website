...
export const useMembersStore = defineStore("members", () => {
  ...
  const changedCount = computed(
    () => new Set(changedCoords.value.map((c) => c.rowId)).size,
  );
  ...
  const membersMap = computed(
    () => new Map(members.value.map((m) => [m.id, m])),
  );
  ...
  function _updateChangedCoords(
    rowId: number,
    field: string,
    isBackToOriginal: boolean,
  ) {
    const idx = changedCoords.value.findIndex(
  ...
  function updateMemberField(
    ...
    const isBackToOriginal =
      JSON.stringify(member[f]) === JSON.stringify(original[f]);
  ...
  function addArrayItem(rowId: number, field: string) {
    ...
    if (member && Array.isArray(member[f])) {
      ...
      const isBackToOriginal =
        !!original && JSON.stringify(member[f]) === JSON.stringify(original[f]);
      _updateChangedCoords(rowId, field, isBackToOriginal);
    }
  }
